...
function M.get_tailwind_named_color_value(color)
	...
	if end_index then
		tailwind_color_name =
			string.sub(tailwind_color_name, end_index + 1, string.len(tailwind_color_name))
	end
...
function M.get_css_var_color(color, row_offset)
	...
	local var_position =
		buffer_utils.get_positions_by_regex(var_patterns, 0, vim.fn.line("$"), 0, row_offset)

	local hsl_without_func_color =
		string.match(var_position[1].value, patterns.hsl_without_func_regex)
...
---@param color string
---@return string|nil
function M.get_background_color(color)
	return ("#" .. string.format("%06x", vim.api.nvim_get_hl(0, { name = "Normal" }).bg))
end

function M.get_inverted_background_color(color)
	return (
		"#"
		.. string.format(
			"%06x",
